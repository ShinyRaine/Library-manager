...
userCollector.checklogin = function (req, res) {
	...
	if (token) {
		try {
			...
			if (e.name="TokenExpiredError") {
				res.json({ code:"error",message: "登录过期，请重新登录" })
			}
		}
	} else {
...
userCollector.checkManage = function (req, res) {
	...
	if (token) {
		try {
			const decoded = jwt.verify(token, key)
			console.log(decoded);
			return res.json({code:"success", manage: decoded.manage })
		} catch (e) {
			console.log(e)
			if (e.name="TokenExpiredError") {
				res.json({ code:"error",message: "登录过期，请重新登录" })
				...
				User.findOne({name: req.body.name}, (err, user) => {
					...
					if (user) {
						...
						user.save((err) => {
							if (err) {
								console.log(err)
								res.json({code:"error", message: '服务器错误'})
							}
							res.json({code:"success", message: "修改成功"})
						})
					} else {
						res.json({code:"error", message: '未找到用户'})
					}
				})
			} else {
				res.json({code: "error", message: "权限不足，请联系管理员修改权限"})
			}
		} catch (e) {
			console.log(e)
			res.json({code: "error", message: '服务器错误'})
